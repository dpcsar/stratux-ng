...
func TestFixtures_Dump1090AircraftJSON_ToStoreSnapshot(t *testing.T) {
	path := filepath.Join("testdata", "dump1090-aircraft.json")
	...
	s.UpsertMany(now, ParseDump1090FAAircraftJSON(json.RawMessage(data)))
	...
	icao1, _ := gdl90.ParseICAOHex("ABCDEF")
	...
	// Ensure trimming behavior.
	if seen[icao1].Tail != "TEST123" {
		t.Fatalf("unexpected tail for ABCDEF: %q", seen[icao1].Tail)
	}
}
